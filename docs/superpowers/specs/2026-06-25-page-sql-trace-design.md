...
로그인 사용자가 Nexacro 화면에서 실행한 조회 SQL을 사용자와 페이지별로 Redis에 저장한다. SQL 목록 조회 시 현재 로그인 사용자의 해당 페이지 로그만 반환하고, 작성 정지 중에는 새 로그를 기록하지 않으며, 초기화 이후에는 이전 로그를 다시 표시하지 않는다.

## Confirmed Behavior

- 로그 소유 키는 `로그인 username + uiId`다.
...
- 초기화는 현재 로그인 사용자와 페이지에 해당하는 Redis 값을 삭제한다.
- 목록 API는 Redis get 결과에 포함된 `QUERY`를 반환한다.
...
```json
{"eventType":"QUERY","username":"user1","requestId":"2de4a7d7-1453-4652-85dd-ef8ddfa57467","uiId":"page01","occurredAt":"2026-06-25T14:20:31.245+09:00","sqlElapsedMillis":18,"sql":"select * from admin_menu where menu_name = '관리'"}
```

TIMING 예시:

```json
{"eventType":"TIMING","username":"user1","requestId":"2de4a7d7-1453-4652-85dd-ef8ddfa57467","uiId":"page01","occurredAt":"2026-06-25T14:20:31.300+09:00","clientApiElapsedMillis":35.2,"clientTotalElapsedMillis":48.7}
```

Redis 접근 규칙:

- 프로세스 내부 append와 조회를 같은 잠금으로 직렬화한다.
- 현재 사용자와 페이지의 Redis 값에 포함된 `QUERY`만 반환한다.
- 같은 `requestId`의 `TIMING`을 SQL 행에 결합한다.
- 손상된 행과 사용자 정보가 없는 과거 형식 행은 건너뛴다.
- Redis 저장 실패는 업무 조회 API를 실패시키지 않는다.
...
```http
GET /api/sql-logs?uiId=page01
```

현재 로그인 사용자와 `uiId`가 일치하고 최근 초기화 이후인 SQL을 모두 반환한다.
...
서버는 현재 로그인 사용자 소유의 동일 `requestId + uiId` QUERY가 있을 때만 TIMING을 기록한다.

### Clear

```http
DELETE /api/sql-logs?uiId=page01
```

현재 로그인 사용자와 `uiId`에 대한 Redis 값을 삭제한다. 다른 사용자와 다른 페이지에는 영향이 없다.

## Nexacro Flow

1. 조회 시작 시 클라이언트 시간을 기록한다.
2. 업무 API 요청에 `X-Ui-Id`, `X-Sql-Capture-Paused`를 전달한다.
3. 업무 응답의 `X-Request-Id`를 읽는다.
4. 작성 정지가 아니면 클라이언트 API 시간과 전체 시간을 timing API로 보낸다.
5. 쿼리 목록 버튼은 `GET /api/sql-logs?uiId=...`를 호출한다.
6. 작성 정지 상태여도 목록 API는 호출 가능하며 기존 누적 로그가 보인다.
7. 초기화 버튼은 `DELETE /api/sql-logs?uiId=...`를 호출한 뒤 그리드를 비운다.
...
- user1과 user2의 같은 uiId 로그가 서로 분리된다.
- 작성 정지 요청은 QUERY 이벤트를 남기지 않는다.
- 작성 정지 상태의 목록 조회는 기존 로그를 반환한다.
- 초기화 후 이전 QUERY가 반환되지 않는다.
- 초기화 후 새 QUERY만 반환된다.
- 초기화 delete 이후에는 같은 사용자와 페이지의 이전 조회 결과가 반환되지 않는다.
